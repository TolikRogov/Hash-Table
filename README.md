...
Хэш-таблица (Hash Table) — это структура данных, которая реализует ассоциативный массив; ассоциативный массив — это абстрактный тип данных, который сопоставляет ключи со значениями. Хэш-таблица использует хэш-функцию для вычисления числа (хэш/hash) по ключу, которое указывает в какой слот (bucket) попадет ключ со своими значениями. Во время поиска ключ хэшируется, и полученный хэш указывает, где хранится соответствующее значение. Бывают случаи, когда хэш-функция выдает один и тот же хэш для разных ключей - это называется коллизией. В результате коллизии в одном слоте (bucket-е) находятся несколько разных элементов, что влечет
...
1.  **Приборная погрешность**
    ...
    | :--------------: | :----: | :----: | :----: | :----: | :----: | :----: | :----: | :----: | :----: | :----: |
    | t, $10 ^ {-4}$ c | 0,0061 | 0,0072 | 0,0080 | 0,0076 | 0,0059 | 0,0036 | 0,0064 | 0,0035 | 0,0049 | 0,0033 |
